...
|---|---|
| **Baseline covariates** | Demography (Age, sex, race, region, enroll_days?? ), clinical history ( prior AKI, heart failure,sepsis,  obstruction, CKD, cirrhosis, portal_htn??, esld??, HIV, diabetes, hypertension, cancer, BMI, overweight_obese??, smoking, alcohol, substance use), concomitant medications (NSAIDs, ACE/ARBs, diuretics, aminoglycosides, contrast agents, statins, aspirin, beta blockers, ccb, ART, chemotherapy) ,|
| **Treatment assignment** | SOF vs. non-SOF, with confounding by CKD, cirrhosis, HIV, age, cancer, diabetes, BMI and (when `complexity=TRUE`) nonlinear covariate interactions |
...
<!-- JOY:  I think its good to retain because its also a clinically relevant estimand. Was done in the primary study analysis and so good to demonstrate how best and when to or not to go after it (How can weighting help reduce the bias (model the switching hazard)) -->
...
<!-- JOY This estimand fundamentally seeks to answer the question, what is the effect of drug A vs B on the risk of badness for my patients while they take it for the time they are able to take it while the hypothetical estimand asks what would the effect of drug A vs drug B be if vereyine completed the intened full course of therapy ie everyone was able to tolerate to the end -->
:::
...
<!-- JOY : Is the cox model analogy of the hypothetical estimand censor at switching ?
YES: Conceptually, we would estimate the hypothetical estimand and while on trt estimand the same way using a cox regression.  With the assumption that switching is non informative. if switching is informative, both these are biased -->
...
# True 180-day risks
true_risk_1 <- mean(truth_a1$event == 1 & truth_a1$follow_time <= tau) #P(T_1<=tau)
true_risk_0 <- mean(truth_a0$event == 1 & truth_a0$follow_time <= tau) #P(T_0<=tau)
