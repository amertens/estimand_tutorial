...
*Counterfactual:* This is a conditioning estimand rather than a pure intervention. The target is the risk of renal failure by 180 days conditional on not switching: $\psi^{WOT} = E[Y^{a}(180) \mid S^{a} > 180]$, where $S^{a}$ is the potential switching time under assignment $a$.

*Truth:* Generate 500,000 all-treated and all-control subjects with switching enabled. Restrict to subjects whose switch time exceeds 180 days (non-switchers). Compute 180-day risk within this subpopulation. This produces a conditional risk among subjects who would tolerate the assigned treatment for the full follow-up — a selected subpopulation that may differ systematically from the full population.

*LMTP implementation:* Data are derived via `derive_estimand("while_on_treatment")`, which censors follow-up at switch time. Treatment is modelled as time-varying, and the static binary intervention holds treatment constant. The Y and C columns reflect censoring at switch, so LMTP estimates the marginal risk under sustained treatment among subjects not yet censored by switching. This is an intervention-based approximation to the conditioning-based WOT estimand; the two coincide when the no-switch intervention is close to the natural course for non-switchers.
...
*LMTP implementation:* Two analyses are reported. First, an approximation that restricts to observed non-switchers (subjects with `switched == 0`), which is a convenience proxy that systematically selects lower-risk patients. Second, an oracle analysis that restricts to true never-switchers (subjects with `never_switcher == 1`), which is valid for simulation but not available in practice. In both cases, treatment is baseline-only (`time_varying_trt = FALSE`), since non-switchers and never-switchers have constant treatment throughout.
...
The LMTP treatment specification differs by estimand:

- **Treatment-policy:** A single baseline treatment variable. The intervention sets $A = 1$ or $A = 0$ at baseline only. Post-baseline switching is part of the natural course, handled by the LMTP censoring model.
- **No-switch and while-on-treatment:** Time-varying treatment ($A_1, \ldots, A_{13}$), reflecting the observed switching trajectory. The static binary intervention overrides all $A_j$ to hold treatment constant, directly implementing the no-switch counterfactual.
- **Composite:** A single baseline treatment variable. Switching is encoded in the outcome (Y columns), not in the treatment nodes.
- **Principal stratum:** A single baseline treatment variable, applied to the subset of never-switchers.

This distinction is critical. Intervening on time-varying treatment to hold it constant is a no-switch intervention. Using baseline-only treatment and letting the censoring model handle switching is a treatment-policy analysis. Confusing the two leads to estimand–estimator mismatch.
...
The treatment-policy and no-switch estimands are distinguished in this simulation by different LMTP treatment specifications: baseline-only for treatment-policy (letting switching occur naturally under the censoring model) versus time-varying with hold-constant intervention for no-switch. Whether these two specifications produce meaningfully different estimates depends on the strength of the switching effect on subsequent outcomes. In the current DGP, the post-switch hazard change is modest (HR shifts from 0.90 to 1.0 for active-to-comparator switchers), so the treatment-policy and no-switch truths are nearly identical.

The while-on-treatment estimand is conceptually a conditioning estimand (condition on not switching), but the LMTP implementation approximates it as an intervention estimand (intervene to prevent switching) on data censored at the switch time. The truth is computed as the conditional risk among counterfactual non-switchers. The intervention-based and conditioning-based quantities coincide when the no-switch intervention is close to the natural course for non-switchers, but they are conceptually distinct.

The composite LMTP uses baseline-only treatment with the composite outcome (renal failure or switching) encoded in the Y columns. Switching is no longer a treatment node; the censoring model adjusts only for administrative censoring. The clinical interpretability of the composite estimand remains limited because it conflates drug tolerability with drug safety.

For the principal stratum, the true-stratum LMTP analysis is an oracle that requires paired potential switching outcomes under both treatments — available in simulation but not in practice. The observed-non-switcher analysis is a convenience proxy that selects a different (generally lower-risk) subpopulation.
