...
This pattern---where the treatment-policy effect is attenuated relative to the no-switch effect---is common in oncology trials with treatment crossover. Patients randomised to control who cross over to the experimental treatment upon progression experience delayed outcomes, reducing the observed treatment-policy survival difference relative to what would have been observed without crossover (Latimer et al. 2014). The hepatitis B setting studied here produces a smaller TP--NS gap because the hazard change upon switching is modest and the switching rate is moderate, but the conceptual distinction remains important for interpreting the clinical question each estimand addresses.
...
km_combined <- list(
  "Full follow-up" = sf_naive,
  "Censored at switch" = sf_cs
)

p_km <- ggsurvplot_list(
  km_combined,
  data = dat_km,
  risk.table = FALSE,
  conf.int = TRUE,
  palette = c("steelblue", "tomato"),
  legend.labs = c("Comparator", "Active"),
  xlab = "Days", ylab = "Survival probability",
  ggtheme = theme_minimal(base_size = 11)
)
arrange_ggsurvplots(p_km, ncol = 2)
```
...
For the treatment-policy estimand, LMTP models treatment as time-fixed at baseline; the censoring model adjusts for informative follow-up loss due to switching. For the no-switch and while-on-treatment estimands, treatment is modelled as time-varying (one column per bin reflecting observed switching), and the static binary intervention holds treatment constant across all bins---directly implementing the "no switching" counterfactual within the LMTP framework. The degree of alignment in each case depends on correct specification of the censoring and outcome models.
...
The second source of mismatch is how switching is handled. The censor-at-switch Cox analysis selectively removes high-risk person-time, attenuating the apparent treatment effect relative to the baseline-treatment model. The degree of attenuation increases with switching intensity, as shown in the support diagnostic section. Neither Cox analysis distinguishes between the treatment-policy and no-switch estimands in a principled way.

LMTP targets a marginal risk contrast more closely aligned with the intervention-defined estimand. The degree of alignment and the residual finite-sample bias depend on the temporal resolution of the time bins, the flexibility of the Super Learner library, and the event rate. Under the current specification (biweekly bins, `SL.mean` + `SL.glm` + `SL.bayesglm`, 2-fold CV), some residual bias is expected. Coverage and bias values should be interpreted in light of the limited number of simulation iterations (20) and the minimal learner library.
