...
```{r}
#| label: tbl-decision-early
#| tbl-cap: "Practical guide to estimand selection in time-to-event safety analyses with informative treatment switching. Coverage values are from the 200-iteration L-TMLE simulation reported in §Simulation Results."

decision_early <- tibble::tibble(
  ...
  Caveat = c(
    ...
    "Most demanding longitudinal nuisance estimation; coverage drops to 87% even at 11% switching. When switching exceeds ~40–50% in key subgroups, consider a calibrated stochastic intervention or step down to treatment-policy or competing-risks while-on-treatment"
  )
)
kable(decision_early, format = "pipe")
```
...
alignment <- tibble::tibble(
  ...
  `L-TMLE specification` = c(
    "Baseline treatment intervention; switching is part of the natural course; censoring model adjusts for informative loss to follow-up",
    "Time-varying treatment intervention holding $A_j$ constant at every bin so the longitudinal product of conditional switching probabilities is targeted directly",
    "Baseline treatment intervention; switching absorbed into the outcome as a competing event; same positivity as treatment-policy",
    "Baseline treatment intervention; outcome redefined as renal failure or switching, whichever first",
    "Baseline treatment intervention applied to the oracle never-switcher subset, i.e., subjects who would not switch under *either* baseline arm (identified in simulation via paired potential switching outcomes); this is the latent stratum, not the observed-did-not-switch subgroup"
  ),
...
```{r}
#| label: tbl-lmtp-singleton
#| include: false
# Single-dataset L-TMLE estimates retained for the workspace cache but not
# tabulated in the manuscript; the 200-iteration simulation tables that
# follow carry the substantive evidence.
...
#| tbl-cap: "Point estimates across the three support scenarios (single dataset, N = 5,000). Inference is not displayed here because each cell is a single-dataset estimate intended to show qualitative behaviour as switching intensity increases; statistical-precision evidence appears in the 200-iteration simulation tables (Table @tbl-sim-main onward)."
...
```{r}
#| label: tbl-sim-supp
#| tbl-cap: "Simulation results for while-on-treatment, composite, and principal stratum strategies (200 iterations, N = 10,000). Truth values: while-on-treatment RD = +0.064, RR = 1.26, HR = 1.51; composite RD = +0.183, RR = 1.46, HR = 1.71; principal stratum RD = +0.090, RR = 1.35, HR = 1.34. Same column conventions as Table @tbl-sim-main."
...
```{r}
#| label: fig-sim-supp
#| fig-cap: "Distribution of L-TMLE risk-difference (top row) and risk-ratio (bottom row) estimates for supplementary estimands across 200 iterations. Dashed lines mark the true marginal RD (top) and true marginal RR (bottom) for each estimand, computed by Monte Carlo on a 500,000-subject counterfactual cohort."
#| fig-height: 7
#| fig-width: 10

supp_estimands <- c("while_on_treatment", "composite", "principal_stratum")

sim_supp_long <- sim_results %>%
  filter(converged, estimand %in% supp_estimands,
         grepl("LMTP", method)) %>%
  mutate(estimand_label = estimand_labels[estimand]) %>%
  select(estimand_label, method, risk_diff, risk_ratio) %>%
  tidyr::pivot_longer(c(risk_diff, risk_ratio),
                      names_to = "scale", values_to = "value") %>%
  mutate(scale = factor(scale, levels = c("risk_diff", "risk_ratio"),
                        labels = c("Risk difference", "Risk ratio"))) %>%
  filter(!is.na(value))

truth_supp_long <- bind_rows(
  tibble(scale = "Risk difference",
         estimand_label = unname(estimand_labels[supp_estimands]),
         xint = sapply(supp_estimands, function(e) truth_all[[e]]$true_rd)),
  tibble(scale = "Risk ratio",
         estimand_label = unname(estimand_labels[supp_estimands]),
         xint = sapply(supp_estimands, function(e) truth_all[[e]]$true_rr))
) %>% mutate(scale = factor(scale, levels = c("Risk difference", "Risk ratio")))

if (nrow(sim_supp_long) > 0) {
  ggplot(sim_supp_long, aes(x = value, fill = method)) +
    geom_histogram(alpha = 0.6, bins = 15, position = "identity") +
    facet_grid(scale ~ estimand_label, scales = "free_x") +
    geom_vline(data = truth_supp_long,
               aes(xintercept = xint), linetype = "dashed", color = "red") +
    labs(x = NULL, y = "Count", fill = "Estimator") +
    theme_minimal(base_size = 11) +
    theme(legend.position = "bottom")
}
```
...
```{r}
#| label: tbl-markov
#| tbl-cap: "Bias and 95% CI coverage for the L-TMLE no-switch and while-on-treatment estimands under full-history vs Markov (k=0) nuisance fits, 50 iterations per scenario."
...
```{r}
#| label: tbl-depcensor
#| tbl-cap: "L-TMLE risk-difference bias and coverage by estimand under informative vs non-informative administrative censoring, 50 iterations per scenario."
...
#| tbl-cap: "DGP parameters used in the main simulation."

dgp_params <- tibble::tibble(
  Parameter = c("Baseline hazard", "Early treatment HR", "Late treatment HR", "Change-point",
                "Baseline switching hazard", "log-HR (treatment on switching)", "log-HR (CKD on switching)",
                "Non-linear interactions", "Informative censoring",
                "Event rate (180d)", "Switch rate (180d)",
                "Switch rate (TDF, CKD)", "Never-switcher %"),
...
```{r}
#| label: tbl-ipcw-supp
#| tbl-cap: "Production-quality time-varying IPCW Cox vs L-TMLE on the same observed switching data, no-switch estimand (50 iterations, N = 10,000)."
#| eval: !expr file.exists(here::here("results", "sim_ipcw_supplement.rds"))

ipcw <- readRDS(here::here("results", "sim_ipcw_supplement.rds"))
truth_hr_ns <- truth_all$no_switch$true_hr
truth_rd_ns <- truth_all$no_switch$true_rd

ipcw_cox <- ipcw %>% filter(converged, !is.na(hr)) %>%
  group_by(method) %>%
  summarise(
    n = n(),
    `Mean HR` = mean(hr),
    `HR bias` = mean(hr - truth_hr_ns),
    `HR cov.` = mean(hr_ci_low <= truth_hr_ns & hr_ci_high >= truth_hr_ns),
    `Mean wt` = mean(mean_w),
    `99th wt` = mean(p99_w),
    `ESS`     = mean(ess),
    .groups = "drop"
...
ipcw_lmtp <- ipcw %>% filter(converged, !is.na(risk_diff)) %>%
  group_by(method) %>%
  summarise(
    n = n(),
    `Mean RD` = mean(risk_diff),
    `RD bias` = mean(risk_diff - truth_rd_ns),
    `RD cov.` = mean(ci_low <= truth_rd_ns & ci_high >= truth_rd_ns),
    .groups = "drop"
  )

knitr::kable(ipcw_cox, digits = 3, format = "pipe",
             caption = "IPCW Cox vs true marginal no-switch HR (1.48).")
cat("\n")
knitr::kable(ipcw_lmtp, digits = 3, format = "pipe",
             caption = "L-TMLE on same data vs true no-switch RD (+0.091).")
```
...
```{r}
#| label: tbl-tv-lmtp
#| tbl-cap: "L-TMLE risk-difference performance under the time-varying-confounder DGP, both calibrations (pure exogenous L(t); treatment-confounder feedback A(t-1) influencing L(t)). 200 iterations, N = 3,000. Monte Carlo SE on bias approximately 0.007-0.008; on coverage approximately 0.02. Baseline-only: adjustment limited to baseline covariates; +L(t): additional adjustment for the time-varying marker at every bin."
#| eval: !expr file.exists(here::here("results", "sim_tv_all_summary.rds"))

tv_all <- readRDS(here::here("results", "sim_tv_all_summary.rds"))
lmtp_tbl <- tv_all$lmtp %>%
  mutate(spec = ifelse(grepl("\\+L", method), "+L(t)", "baseline only"),
         estimand = recode(estimand,
                           "treatment_policy" = "TP",
                           "no_switch" = "no-switch",
                           "while_on_treatment" = "WOT",
                           "composite" = "composite"),
         variant = recode(variant, "pure" = "exogenous L(t)",
                          "fb" = "A→L feedback")) %>%
  select(`DGP variant` = variant, Estimand = estimand, Spec = spec,
         `Truth (RD)` = truth, `Mean RD` = mean_rd, `Bias` = bias,
         `RMSE` = rmse, `Cov.` = cov) %>%
  arrange(`DGP variant`, Estimand, Spec)
knitr::kable(lmtp_tbl, digits = 3, format = "pipe")
```

```{r}
#| label: tbl-tv-cox
#| tbl-cap: "Cox estimand-aligned HR estimates under the time-varying-confounder DGP, both calibrations. Each row's true marginal RD (on the risk-difference scale) is shown for context; Cox is judged on its native HR scale. 100% of CI intervals exclude HR=1, but the HR estimates substantially exceed the marginal effect the corresponding risk difference would imply."
#| eval: !expr file.exists(here::here("results", "sim_tv_all_summary.rds"))
...
cox_tbl <- tv_all$cox %>%
  mutate(estimand = recode(estimand,
         ...
         variant = recode(variant, "pure" = "exogenous L(t)",
                          "fb" = "A→L feedback")) %>%
  select(`DGP variant` = variant, Estimand = estimand,
         `Cox method` = method, `Mean HR` = mean_hr,
         `Mean log HR` = mean_log_hr, `CI excl. HR=1 (%)` = pct_excl_null) %>%
  arrange(`DGP variant`, Estimand, `Cox method`)
knitr::kable(cox_tbl, digits = 3, format = "pipe")
```
...
```{r}
#| label: tbl-ph-cox
#| tbl-cap: "Cox hazard-ratio performance under a proportional-hazards-satisfied DGP (conditional HR = 2.0 throughout). Marginal HR truths differ from 2.0 by non-collapsibility alone."
#| eval: !expr file.exists(here::here("results", "sim_ph_summary.rds"))

ph <- readRDS(here::here("results", "sim_ph_summary.rds"))
ph_cox <- ph$cox %>%
  mutate(estimand = recode(estimand,
                           "treatment_policy" = "TP",
                           "no_switch" = "no-switch",
         ...
         method = recode(method,
                         "Cox naive" = "Cox naive",
                         "Cox censor-at-switch" = "Cox censor-at-switch",
                         "Cox IPCW" = "Cox IPCW")) %>%
  select(Estimand = estimand, `Cox method` = method,
         `True marg HR` = true_marg_hr, `Mean HR` = mean_hr,
         `HR bias` = bias_hr, `HR cov.` = hr_cov) %>%
  arrange(Estimand, `Cox method`)
knitr::kable(ph_cox, digits = 3, format = "pipe")
```

```{r}
#| label: tbl-ph-lmtp
#| tbl-cap: "L-TMLE risk-difference performance under the proportional-hazards-satisfied DGP."
#| eval: !expr file.exists(here::here("results", "sim_ph_summary.rds"))
...
ph_lmtp <- ph$lmtp %>%
  mutate(estimand = recode(estimand,
                           ...
                           "while_on_treatment" = "WOT",
                           "composite" = "composite")) %>%
  select(Estimand = estimand, `LMTP method` = method,
         `True RD` = true_rd, `Mean RD` = mean_rd,
         `RD bias` = bias, RMSE = rmse, `RD cov.` = rd_cov) %>%
  arrange(Estimand)
knitr::kable(ph_lmtp, digits = 3, format = "pipe")
```
...
```{r}
#| label: tbl-stochastic
#| tbl-cap: "L-TMLE shift intervention (delta = 0.5) and no-switch anchor vs the half-switching truth (RD = +0.0807). 50 iterations, N = 10,000."
#| eval: !expr file.exists(here::here("results", "sim_stochastic_arm.rds"))

sto <- readRDS(here::here("results", "sim_stochastic_arm.rds"))
true_rd_half <- unique(sto$true_rd)
sto_summary <- sto %>% filter(converged, !is.na(risk_diff)) %>%
  group_by(method) %>%
  summarise(
    n     = n(),
    `Mean RD` = mean(risk_diff),
    `Bias`    = mean(risk_diff - true_rd_half),
    `RMSE`    = sqrt(mean((risk_diff - true_rd_half)^2)),
    `Cov.`    = mean(ci_low <= true_rd_half & ci_high >= true_rd_half),
    .groups = "drop"
  )
knitr::kable(sto_summary, digits = 3, format = "pipe")
```
